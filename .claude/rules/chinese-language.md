...
> 本规则为默认语言约束。更具体规则（如项目已有规范、用户明确要求）优先级更高。
> 适用于：回复文字、代码注释、commit message。
> 不适用于：技术术语、代码标识符、错误日志原文。
...
4. **代码标识符（例外）**：变量/函数/类型/包名、数据库字段、JSON key、URL 路径、枚举值保持英文。
