...
> 详见 `claude-code-defensive.md` - 严格禁止 #8「用全局开关掩盖错误」
...
```bash
# 在项目里 grep 是否有重名
grep -rn "@Bean(name = \"priceBatchUpdateExecutor\"" src/
grep -rn "class PriceBatchUpdateExecutor" src/
...
**防幻觉兜底策略**：如果按上述方案排查同名 Bean 未果，**绝对禁止** 盲目推测或尝试覆盖逻辑，必须立即使用 `ask_user` 工具要求用户提供完整的 `BeanDefinitionStoreException` 启动报错堆栈。
