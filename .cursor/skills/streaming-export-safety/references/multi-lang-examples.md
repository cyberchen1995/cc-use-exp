...
```java
EasyExcel.write("orders.xlsx", OrderDTO.class)
    .registerWriteHandler(new LongestMatchColumnWidthStyleStrategy())
    .sheet("orders")
    .doWrite(() -> {
        return orderRepo.findAll(PageRequest.of(pageNo++, 1000))
            .map(this::toDTO)
            .getContent();
    });
```
...
with pd.ExcelWriter("orders.xlsx", engine="openpyxl") as writer:
    ...
    while True:
        ...
        if not orders:
            break
        df = pd.DataFrame([{"id": o.id, "amount": o.amount} for o in orders])
        df.to_excel(writer, sheet_name="orders",
                    startrow=start_row, header=(page == 0), index=False)
        start_row += len(df)
        page += 1
```
...
> 异步任务的完整状态机、超时、幂等设计见 `async-task-pattern` skill
