...
> 与 `cc-query-performance-safety` 配合：先用后者把同步性能压到能跑完的范围；只有"再优化也压不到 10s 以内"时，才走本规范。
...
> 完整错误/正确代码示例见 `cc-frontend-dev` skill - 陷阱 #3「useEffect 双轮询冲突」
...
| 同步接口加 `@Async` 注解（self-invocation） | 不走代理，注解不生效 → 见 `cc-java-dev` skill 「Spring self-invocation 陷阱」 |
...
```
> 📋 本回复遵循：`cc-async-task-pattern` - [章节]
```
