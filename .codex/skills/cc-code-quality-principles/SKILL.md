---
name: cc-code-quality-principles
...
| # | 原则 | 一句话 | 衡量指标 | 详细案例 |
|---|------|--------|---------|---------|
| 1 | 模块大小适中 | 单文件控制在职责边界内 | 行数/职责数 | 引用 [cc-size-check](../cc-size-check/) |
...
| 5 | 作用域内聚 | 改 A 不波及 B | 跨模块副作用 | 引用 [cc-refactor-safety](../cc-refactor-safety/) |
...
> 主要执行交给 `cc-size-check` skill；本 skill 仅作总纲提示。
...
> 主要执行交给 `cc-refactor-safety` 与 `cc-multi-tenant-safety` skill。
...
|------|---------------|--------------|
| 1 大小适中 | `cc-size-check` | 触发提示 |
| 2 调用深度 | 无独立 skill | **本 skill 主导** |
| 3 扇入扇出 | 无独立 skill | **本 skill 主导** |
| 4 边界清晰 | `cc-api-design-safety` | 决策对照 |
| 5 作用域内聚 | `cc-refactor-safety` / `cc-multi-tenant-safety` | 触发提示 |
| 6 可预测 | `cc-time-zone-safety` / `cc-redis-safety` / `cc-query-performance-safety` | **本 skill 主导，专项 skill 处理具体陷阱** |
...
```
> 📋 本回复遵循：`cc-code-quality-principles` - [原则编号]
```
